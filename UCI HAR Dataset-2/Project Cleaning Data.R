...
train <- read.table("./UCI HAR Dataset-2/train/X_train.txt") #Features data
colnames(train) <- features$V2  #descriptive column names for data (STEP 4)
y_train <- read.table("./UCI HAR Dataset-2/train/y_train.txt") #activity labels
train$activity <- y_train$V1
subject_train <- read.table("./UCI HAR Dataset-2/train/subject_train.txt") #subjects
train$subject <- factor(subject_train$V1)

# 3. Reading Test Data
test <- read.table("./UCI HAR Dataset-2/test/X_test.txt")
...
write.table(dataset.tidy, "tidy.txt", row.name = FALSE)
